...
export function useFileAssetUrls(referenceIds: Array<string | undefined>) {
  const client = useClient({apiVersion: API_VERSION})
  const ids = useMemo(() => Array.from(new Set(referenceIds.filter(Boolean) as string[])), [referenceIds.join('|')])
  ...
  useEffect(() => {
    ...
    let active = true
    client.fetch<FileAsset[]>('*[_id in $ids]{_id,url}', {ids}).then((assets) => {
      if (!active) return
      setUrls(Object.fromEntries(assets.flatMap((asset) => asset.url ? [[asset._id, asset.url]] : [])))
    }).catch(() => {
      if (active) setUrls({})
    })
