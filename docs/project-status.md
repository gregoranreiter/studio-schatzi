...
The site now has a purpose-built Sanity Studio and a populated, separate Sanity production dataset. Astro reads that content during static builds. The repository contains isolated Cloudflare Worker configurations for the public site and CMS. Both pass local packaging checks and are live for review at temporary `fragrant-buffer.workers.dev` URLs. The temporary account must be claimed within Cloudflare's one-hour window to preserve those previews; permanent deployment to the existing account that contains BauConsult still requires Cloudflare CLI authorization on the development computer.
...
- Authorize Wrangler against the intended permanent Cloudflare account, deploy and protect the two Workers there, connect Workers Builds, and test the Sanity deploy hook.
