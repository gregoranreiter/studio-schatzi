...
The project archive uses a uniform grid with 16:9 covers and stacked captions: four columns from 1200px, two columns from 761px to 1199px, and one column at 760px and below. Archive descriptions align at the top, with 1.25rem between the title area and description. The four-column layout reserves two lines for titles so their descriptions start together. Both pages use the same project data and `ProjectCard` component. Homepage caption spacing scales from 1.5rem to 2.4rem; archive captions retain 2.4rem above them.
...
The header follows the same four-column grid, with Projekte, Leistungen, and Studio centered as a group across the middle two columns. The logo stays left and Kontakt aligns to the right page inset. Below 480px, the logo and Kontakt share the first row, with the three page links centered beneath them. All navigation items use the shared small text size.
...
Every page uses the shared black footer with white text in `SiteFooter.astro`. On the homepage it follows the project list and archive link directly, without a separate statement or services teaser. The archive link is a yellow pill across columns 2–3 with the unchanged medium text size, becoming a centered, content-sized button on mobile. The footer fills at least one viewport and leaves space below the fixed header. A compact, full-width loop of seven white client logos sits below the contact details, directly above the imprint and copyright row. The original logo files stay unchanged; CSS renders them white. The band pauses on hover or via its checkbox and becomes a static wrapping row with reduced motion. Original logo sources are recorded in `docs/client-logos.md`.

The footer headline spans columns 1–3. Email, phone, and the street address appear in a medium-size contact block in columns 3–4 on the next row, stacking on small screens. The legal label and copyright align to the same grid at the bottom. The legal label remains unlinked until approved text or a destination is supplied. The default headline comes from the positioning service; service detail pages pass their own headline into the same footer and no longer render a separate CTA section.
