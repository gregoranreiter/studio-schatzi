...
| `studio/studioComponents/` | Homepage and project-gallery compositions, plus layout choice controls. |
...
On desktop, service titles rest at the bottom of each column on a continuous yellow background that stays unchanged on hover. Hover and keyboard focus reveal the trailing arrow and raise the title beneath the service's large description without adding an underline. The arrow reserves its space while hidden. Matter.js drives the titles, headline masks, and image position through one on-demand engine in `src/lib/physical-motion.ts`. Titles lift with an immediate spring impulse and a slight elastic stretch, then stop cleanly at the raised destination without an upper bounce or impact compression. Leaving releases the title at its current height, stops the upward pull, and lets stronger gravity accelerate it toward the baseline. A typical full-height drop lands in under 250ms, with a more visible floor rebound and compression. Re-hovering catches the falling title without resetting its position or momentum. Stretch stays within 3.5% and compression within 4.5%; the title returns to its natural shape at rest. The raised destination follows the description's rendered height and leaves a small gap. CSS provides the no-script fallback.

The large description stays fixed at full opacity. A single physical progress value moves both clipping masks, concealing the previous text while revealing the next. Between services, the conceal edge leads the reveal edge by 20px, leaving a strip of the yellow page background between the old and new text. Moving right sweeps from left to right, and moving left reverses the sweep. Entering the four-column service area starts with a zero-width seam at the first hovered column's left edge and expands in both directions. Leaving the entire service area reverses that motion and closes back to the same column edge. The edge is measured relative to the headline's page-wide bounds and gutters. Initial keyboard focus uses the column's horizontal center as a stable fallback. Interrupted reveals retain their current mask for concealment, re-hovering catches an unfinished close in place, and rapid switches discard older outgoing text. Moving within a column does not restart the wipe. Clicking a service preserves its active headline and raised title while the page-transition cover enters; swap cleanup happens only once the old page is covered. Loss of window focus and layout changes still stop the engine and remove outgoing text. Reduced motion settles the titles and masks immediately. The engine uses fixed 120Hz simulation steps, caps time after a suspended tab, and requests no frames while at rest. Tests exercise the real Matter engine, including retargeting, refresh rates, and cleanup.
